...
import xyz.ineanto.noencryptionx.utils.CommentedConfiguration;
...
public class ConfigurationHandler {
    ...
    public static class Config {
        ...
        private static boolean loadConfig() {
            ...
            if (!config.load()) {
                NoEncryptionX.logger().log(Level.SEVERE, "Failed to load config.yml");
                return false;
            }
        ...
        public static void printChanges() {
            NoEncryptionX.logger().info("Checking for new config option...");

            if (!newOptions.isEmpty()) {

                newOptions.forEach((root, def) -> NoEncryptionX.logger().info("  " + root + ": " + def));
            } else {
                NoEncryptionX.logger().info("No new config options detected");
            }
        }
    ...
    public static class Notices {
        ...
        private static boolean loadNotices() {
            ...
            if (!notices.load()) {
                NoEncryptionX.logger().log(Level.SEVERE, "Failed to load notices.yml");
                return false;
            }
        ...
        public static void loadAndPrintChanges() {
            NoEncryptionX.logger().info("Checking for important messages...");
            ...
            if (!messages.isEmpty()) {
                ...
                if (!display.isEmpty()) {
                    display.forEach((root, msg) -> {
                        NoEncryptionX.logger().warning("  " + root + ": \"" + msg + "\"");
                        activeMessages.put(root, msg);
                    });

                    NoEncryptionX.logger().info("These messages can be suppressed with /ne suppressnotices");
                } else {
                    NoEncryptionX.logger().info("No important messages detected");
                }
            } else {
                NoEncryptionX.logger().info("No important messages detected");
            }
